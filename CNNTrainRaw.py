...
def train(x_train, y_train, x_test, y_test, input_shape=(DEF_N_ROWS, 6), num_classes=len(motion_names),
          batch_size=DEF_BATCH_SIZE, epochs=DEF_NUM_EPOCH):
    inputs = layers.Input(shape=(150, 6))  # type: ignore

    x = layers.Conv1D(32, kernel_size=5, strides=2, padding='same')(inputs)  # type: ignore
    x = layers.BatchNormalization()(x)  # type: ignore
    x = layers.ReLU()(x)  # type: ignore
    x = layers.Dropout(0.2)(x)  # type: ignore

    x = layers.Conv1D(32, kernel_size=3, strides=2, padding='same')(x)  # type: ignore
    x = layers.BatchNormalization()(x)  # type: ignore
    x = layers.ReLU()(x)  # type: ignore

    x = layers.Conv1D(64, kernel_size=3, strides=2, padding='same')(x)  # type: ignore
    x = layers.BatchNormalization()(x)  # type: ignore
    x = layers.ReLU()(x)  # type: ignore

    x = layers.GlobalAveragePooling1D()(x)  # type: ignore

    x = layers.Dense(32)(x)  # type: ignore
    ...
    checkpoint = callbacks.ModelCheckpoint(DEF_MODEL_NAME, monitor='val_accuracy', save_best_only=True, # type: ignore
...
all_data = np.vstack(file_list)
mean = np.mean(all_data, axis=0)
std = np.std(all_data, axis=0)

np.save('norm_params.npy', {'mean': mean, 'std': std})  # type: ignore
file_list_normalized = normalize_data(file_list, mean, std)

file_list_padded = pad_sequences(file_list_normalized, maxlen=max_len, dtype='float32', padding='post', value=0)
...
model.predict(np.zeros((1, 150, 6)))
model.compile(optimizer=optimizers.Adam(),  # type: ignore
              loss=losses.CategoricalCrossentropy(),  # type: ignore
              metrics=['accuracy'])  # type: ignore
...
# 这里直接使用x_test
x_test_sample = x_test[:10]  # 使用前100个样本作为校准数据集
